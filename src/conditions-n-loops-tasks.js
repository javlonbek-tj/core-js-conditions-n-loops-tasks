...
function convertToRomanNumerals(num) {
  ...
  function convertDigit(number, prefix) {
    const digit = num - number;
    const romanNumeral = convertToRomanNumerals(digit);
    return `${prefix}${romanNumeral}`;
  }

  if (num <= 20) {
    return convertDigit(10, 'X');
  }
  if (num <= 30) {
    return convertDigit(20, 'XX');
  }

  return convertDigit(30, 'XXX');
}
